...
function getPlayers(region = 'global', env = 'g') {
    return new Promise(async resolve => {
        var data = getCache('v1/rankings/'+region+'/players');
        if (data) return resolve(data);
        if (!data) data = await apiFetch('v1/rankings/'+region+'/players');
        if (data && data.hasOwnProperty('statusCode') && data.statusCode == 200) setCache('v1/rankings/'+region+'/players', data, 55, 's');
        return resolve(data);
    });
}
...
window.addEventListener('load', async () => {
    ...
    setInterval(async function() {
        let data = await getPlayers(region, env);
