...
function getPowerPlay(region = 'global', season = 'latest', env = 'g') {
    return new Promise(async resolve => {
        var data = getCache('v1/rankings/'+region+'/powerplay/seasons/'+season);
        if (data) return resolve(data);
        if (!data) data = await apiFetch('v1/rankings/'+region+'/powerplay/seasons/'+season);
        if (data && data.hasOwnProperty('statusCode') && data.statusCode == 200) setCache('v1/rankings/'+region+'/powerplay/seasons/'+season, data, 55, 's');
        return resolve(data);
    });
}
...
window.addEventListener('load', async () => {
    if (env == 'cn' || season !== 'latest') return;
    startProgress();
    setInterval(async function() {
        let data = await getPowerPlay(region, season, env);
